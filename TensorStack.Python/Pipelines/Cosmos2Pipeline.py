...
_stopwatch = None
_pipelineMap = {
    ProcessType.TextToImage: Cosmos2TextToImagePipeline
}
...
def load_vae(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if single_path and single_path.is_file():
        ...
        auto_encoder = Utils.from_component(Cosmos2TextToImagePipeline, "vae", single_path, template_path, _device_map, config.data_type)
        if auto_encoder:
            Utils.trim_memory(True)
            return auto_encoder
