...
from transformers import GemmaTokenizer, T5Gemma2Encoder
...
_stopwatch = None
_pipelineMap = {
    ProcessType.TextToVideo: MotifVideoPipeline,
    ProcessType.ImageToVideo: MotifVideoImage2VideoPipeline
}
...
def load_text_encoder(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    text_encoder = T5Gemma2Encoder.from_pretrained(
        text_encoder_path,
...
#------------------------------------------------
# Load MotifVideoTransformer3DModel
#------------------------------------------------
def load_transformer(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    transformer =  MotifVideoTransformer3DModel.from_pretrained(
        str(transformer_path),
        torch_dtype=config.data_type,
        device_map=_device_map,
...
def load_vae(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if single_path and single_path.is_file():
        ...
        auto_encoder = Utils.from_component(MotifVideoPipeline, "vae", single_path, template_path, _device_map, torch.float32)
        if auto_encoder:
            Utils.trim_memory(True)
            return auto_encoder
...
def generate(
    ...
    # Options
    options = DataObjects.GenerateVideoOptions(**inference_args)
    ...
    # Pipeline Options
    (prompt_embeds, negative_prompt_embeds, prompt_attention_mask, negative_prompt_attention_mask) = _prompt_cache_value
    pipeline_options = {
