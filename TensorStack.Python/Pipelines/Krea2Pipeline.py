...
_stopwatch = None
_pipelineMap = {
    ProcessType.TextToImage: Krea2Pipeline
}
...
def load_vae(config: DataObjects.PipelineConfig, pipeline_kwargs: Dict[str, str]):
    ...
    if single_path and single_path.is_file():
        ...
        auto_encoder = Utils.from_component(Krea2Pipeline, "vae", single_path, template_path, _device_map, config.data_type)
        if auto_encoder:
            Utils.trim_memory(True)
            return auto_encoder
...
def generate(
    ...
    if _prompt_cache_key != prompt_cache_key:
        ...
        with torch.no_grad():
            ...
            if options.guidance_scale > 1:
                negative_prompt_value, negative_prompt_mask = _pipeline.encode_prompt(
                    prompt=options.negative_prompt,
                    device=_pipeline._execution_device
                )

            _prompt_cache_value = (prompt_value, prompt_mask, negative_prompt_value, negative_prompt_mask)
    ...
    # Pipeline Options
    (prompt_embeds, prompt_mask, negative_prompt_embeds, negative_prompt_mask) = _prompt_cache_value
    pipeline_options = {
