...
{
    /// <summary>
    /// PipelineProxy: Proxy between HuggingFace Diffusers/Transformers and C#
    /// </summary>
    public sealed class HuggingFacePipeline : IDisposable
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="HuggingFacePipeline"/> class.
        /// </summary>
        /// <param name="moduleName">Name of the module.</param>
        /// <param name="logger">The logger.</param>
        public HuggingFacePipeline(PipelineConfig configuration, IProgress<PipelineProgress> progressCallback = default, ILogger logger = default)
        {
            _logger = logger;
            _isRunning = true;
            ...
            using (GIL.Acquire())
            {
                _logger?.LogInformation("[HuggingFacePipeline] [Load] Importing pipeline module '{pipelineName}'.", _pipelineName);
        ...
        public void ReloadModule()
        {
            using (GIL.Acquire())
            {
                _logger?.LogInformation("[HuggingFacePipeline] [ReloadModule] Reloading module.");
        ...
        public Task<bool> LoadAsync()
        {
            return Task.Run(() =>
            {
                using (GIL.Acquire())
                {
                    try
                    {
                        _logger?.LogInformation("[HuggingFacePipeline] [Load] Loading pipeline.");
        ...
        public Task<bool> ReloadAsync(PipelineReloadOptions options)
        {
            return Task.Run(() =>
            {
                using (GIL.Acquire())
                {
                    try
                    {
                        _logger?.LogInformation("[HuggingFacePipeline] [Reload] Reloading pipeline.");
        ...
        public Task<bool> UnloadAsync()
        {
            return Task.Run(() =>
            {
                using (GIL.Acquire())
                {
                    try
                    {
                        _logger?.LogInformation("[HuggingFacePipeline] [Unload] Unloading pipeline.");
        ...
        public Task<ImageTensor[]> GenerateImageAsync(GenerateImageOptions options, CancellationToken cancellationToken = default)
        {
            return Task.Run<ImageTensor[]>(() =>
            {
                using (GIL.Acquire())
                {
                    try
                    {
                        _logger?.LogInformation("[HuggingFacePipeline] [Generate] Executing pipeline.");
        ...
        private Task GenerateCancelAsync()
        {
            return Task.Run(() =>
            {
                using (GIL.Acquire())
                {
                    try
                    {
                        _logger?.LogInformation("[HuggingFacePipeline] [GenerateCancel] Canceling generation.");
        ...
        public void Dispose()
        {
            _logger?.LogInformation("[HuggingFacePipeline] [Dispose] Disposing pipeline.");
            _isRunning = false;
            UnbindFunctions();
            _module.Dispose();
                ...
                try
                {
                    ...
                    if (!progressItems.IsNullOrEmpty())
                    {
                        foreach (var progress in progressItems)
                        {
                            ...
                            _logger?.LogDebug("[HuggingFacePipeline] {Progress}", progress);
                        }
                    }
                    ...
                    foreach (var logEntry in LogParser.ParseLogs(logEntries).OrderBy(x => x.Timestamp))
                    {
                        ...
                        _logger?.LogInformation("[HuggingFacePipeline] [{Timestamp}] {Message}", logEntry.Timestamp.ToString("hh:mm:ss:fff"), logEntry.Message);
                    }
                }
                catch (Exception ex)
                {
                    _logger?.LogError(ex, "[HuggingFacePipeline] [NotificationLoop]");
                }
        ...
        private async Task TokenProgressLoop(int refreshRate)
        {
            while (_isRunning)
            {
                ...
                {
                    _logger?.LogError(ex, "[HuggingFacePipeline] [TokenProgressLoop]");
                    await Task.Delay(refreshRate);
                }
            }
        }
        ...
        private Exception HandlePythonException(PythonInvocationException ex)
        {
            if (ex.InnerException is PythonRuntimeException pyex)
            {
                ...
                _logger?.LogError(pyex, "[HuggingFacePipeline] {PythonExceptionType} exception occurred", ex.PythonExceptionType);
            ...
            _logger?.LogError(ex, "[HuggingFacePipeline] {PythonExceptionType} exception occurred", ex.PythonExceptionType);
